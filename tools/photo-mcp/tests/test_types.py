...
def test_manifest_defaults_to_empty_galleries_and_images():
    ...
    m = Manifest(generated=datetime.now(UTC), base_url="https://photos.test.com")
    assert m.galleries == {}
    assert m.images == {}
...
def test_manifest_round_trips_via_json():
    ...
    m = Manifest(
        ...
        galleries={"landscapes": ManifestGallery(title="Landscapes", created=datetime.now(UTC))},
        images={
            "test-img": ManifestImage(
                id="test-img",
                filename="test.jpg",
                path="landscapes/test.jpg",
                gallery="landscapes",
